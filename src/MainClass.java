...
public class MainClass {
	...
	public static TernarySearchTree tst = new TernarySearchTree();
...
//	public static ArrivalTimes ATs = new ArrivalTimes();
...

	{
		...
		if (badInputs == false) {
			...
			boolean exit = false;
			while (exit == false) {
				System.out.println(" 1. Shortest Path bw 2 Stops ");
				System.out.println(" 2. Search a Bus Stop ");
				System.out.println(" 3. Get All Trips ");
				...
				if (inputScanner.hasNextLine()) {
					...
					} else if (inputFunction.equals("1"))// Shortest Paths between 2 Stops
					{
						...
						try {
							...
							Graphs bus = new Graphs(stops, transfers, stopTimes);
							if (!bus.invalidInput) {
	...
	//
	//
	public static String getStopNames(String ID) {
		// The string we will be outputting
		String output = "";
		for (int i = 0; i < stops.size(); i++) {
